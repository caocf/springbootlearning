...
/*	Every input option is tested and parsed. This'll prevent
	...
	function testExchange ( parsed, entry ) {
		parsed.exchange=entry;
	}
	...
	function testOptions ( options ) {
		...
		tests = {
			...
			'exchange': { r: false, t: testExchange }
		};
...
function closure ( target, options ){
	...
	function setHandle ( handle, to, noLimitOption ) {
		...
		/*if(options.exchange&&!options.margin){
			if(trigger){
					if(to<lowerMargin){
						trigger = 0;
						handle = scope_Handles[0];
					}
			}else{
				if(to>upperMargin){
					trigger = 1;
					handle = scope_Handles[1];
				}
			}
		}else*/ if ( scope_Handles.length > 1 ) {
